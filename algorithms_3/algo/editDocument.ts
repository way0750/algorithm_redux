...
cursor position ex, 1, should consider as left of the position
just loop through each operation using for loop (in case needing to short cut return false)
maintain a current cursor position
maintain a global current string for eventual comparision
    in each loop update this string per operation
*/
...
export function isValid(stale, latest, otjson) {
    ...
    for (let i = 0; i < operations.length; i++) {
        const { op, count, chars } = operations[i];
        // skip, delete, insert
        if (op === 'skip') {
            cursorPos += count;
            if (cursorPos >= tempStr.length) return false;
        } else if (op === 'delete') {
            if (count - (tempStr.length - cursorPos) > 1) return false;
            tempStr = tempStr.slice(0, cursorPos) + tempStr.slice(cursorPos + count);
        } else if (op === 'insert') {
            tempStr = tempStr.slice(0, cursorPos) + chars + tempStr.slice(cursorPos);
            cursorPos += chars.length;
        }
    }

    return latest === tempStr;
}
...
describe('', () => {
    ...
    it('1', () => {
        const reuslt = isValid(
            'Repl.it uses operational transformations to keep everyone in a multiplayer repl in sync.',
            'Repl.it uses operational transformations.',
            '[{"op": "skip", "count": 40}, {"op": "delete", "count": 47}]'
        ); // true
        expect(reuslt).to.equal(true);
    });
